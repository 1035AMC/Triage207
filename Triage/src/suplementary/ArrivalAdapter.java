 ...
 * @author Priyank Purohit, arturomenacruz
 *
 */
public class ArrivalAdapter extends ArrayAdapter<Patient> {
	// View lookup cache
	 ...
	 */
	public ArrivalAdapter(Context context, ArrayList<Patient> patients) {
		super(context, R.layout.activity_nurse_triage, patients);
	}
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (lastVisitIndex >= 0)
			viewHolder.healthCard.setText(""
					+ ViewVitalsActivity.timeToString(
							patient.getVisit().get(lastVisitIndex)
									.getArrivalTime()[3], patient.getVisit()
									.get(lastVisitIndex).getArrivalTime()[4]));
		else
