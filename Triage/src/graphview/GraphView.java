...
abstract public class GraphView extends LinearLayout {
	...
	private class GraphViewContentView extends View {
		...
		protected void onDraw(Canvas canvas) {
			...
			if (horlabels == null) {
				...
				Log.w("GraphView",
						"when you use static labels (via setHorizontalLabels) the labels will just be shown exactly in that way, that you have set it. setNumHorizontalLabels does not have any effect.");
			}
			if (verlabels == null) {
				verlabels = generateVerlabels(graphheight);
			} else if (getGraphViewStyle().getNumVerticalLabels() > 0) {
				Log.w("GraphView",
						"when you use static labels (via setVerticalLabels) the labels will just be shown exactly in that way, that you have set it. setNumVerticalLabels does not have any effect.");
			}
		...
		protected void onDraw(Canvas canvas) {
			...
			for (int i = 0; i < verlabels.length; i++) {
				...
				for (int li = 0; li < lines.length; li++) {
					...
					float y2 = y - (lines.length - li - 1)
							* graphViewStyle.getTextSize() * 1.1f;
	...
	protected void drawHorizontalLabels(Canvas canvas, float border,
		...
		for (int i = 0; i < horlabels.length; i++) {
			...
			if (graphViewStyle.getGridStyle().drawVertical()) { // vertical
																// lines
			...
			if (showHorizontalLabels) {
				...
				for (int li = 0; li < lines.length; li++) {
					// for the last line y = height
					float y = (height - 4) - (lines.length - li - 1)
							* graphViewStyle.getTextSize() * 1.1f;
	...
	private String[] generateHorlabels(float graphwidth) {
		...
		for (int i = 0; i <= numLabels; i++) {
			...
			Double temp = Double.valueOf(formatLabel(min
					+ ((max - min) * i / numLabels), true));
			int rounded = (int) Math.round(temp);
			labels[i] = graphSeries.get(0).values[rounded].getDate();

			// labels[i] = values[i].getDate();
		}
		return labels;
	}
